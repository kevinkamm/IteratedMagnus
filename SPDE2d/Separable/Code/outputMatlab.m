...
% Result.EulerRef=addRelError(Result.EulerRef,'Exact',relErrEulerRefEuler);
end
if ~isempty(dtEuler)
Result.Euler=struct;
Result.Euler=addMethod(Result.Euler,ctimeEuler,dtEuler);
Result.Euler=addRelError(Result.Euler,'EulerRef',relErrEulerRefEuler);
end

if ~isempty(relErrEulerRefMCS2)
Result.Magnus2=struct;
Result.Magnus2=addMethod(Result.Magnus2,ctimeMagnus2CS,dT,dtMagnusLog);
Result.Magnus2=addRelError(Result.Magnus2,'EulerRef',relErrEulerRefMCS2);
end

if ~isempty(relErrEulerRefMCS3)
Result.Magnus3=struct;
Result.Magnus3=addMethod(Result.Magnus3,ctimeMagnus3CS,dT,dtMagnusLog);
Result.Magnus3=addRelError(Result.Magnus3,'EulerRef',relErrEulerRefMCS3);
end
